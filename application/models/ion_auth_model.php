...
class Ion_auth_model extends CI_Model
{
	...
	public function hash_password_db($id, $password, $use_sha1_override=FALSE)
	{
		...
		$query = $this->_mysql->select('password, salt')
		                  ->where('id', $id)
		                  ->limit(1)
	...
	public function activate($id, $code = false)
	{
		...
		if ($code !== FALSE)
		{
			$query = $this->_mysql->select($this->identity_column)
			...
			$this->trigger_events('extra_where');
			$this->_mysql->update($this->tables['users'], $data, array($this->identity_column => $identity));
		}
		else
		{
			$data = array(
			...
			$this->trigger_events('extra_where');
			$this->_mysql->update($this->tables['users'], $data, array('id' => $id));
		}


		$return = $this->_mysql->affected_rows() == 1;
		if ($return)
		{
	...
	public function clear_forgotten_password_code($code) {
		...
		$this->_mysql->where('forgotten_password_code', $code);

		if ($this->_mysql->count_all_results($this->tables['users']) > 0)
		{
			$data = array(
			...
			$this->_mysql->update($this->tables['users'], $data, array('forgotten_password_code' => $code));
	...
	public function reset_password($identity, $new) {
		...
		$query = $this->_mysql->select('id, password, salt')
	...
	public function change_password($identity, $old, $new)
	{
		...
		if ($old_password_matches === TRUE)
		{
			...
			$successfully_changed_password_in_db = $this->_mysql->affected_rows() == 1;
	...
	public function username_check($username = '')
	{
		...
		return $this->_mysql->where('username', $username)->count_all_results($this->tables['users']) > 0;
	}
	...
	public function email_check($email = '')
	{
		...
		return $this->_mysql->where('email', $email)
	...
	public function identity_check($identity = '')
	{
		...
		return $this->_mysql->where($this->identity_column, $identity)
		                ->count_all_results($this->tables['users']) > 0;
	}
	...
	public function forgotten_password($identity)
	{
		...
		$this->_mysql->update($this->tables['users'], $update, array($this->identity_column => $identity));
	...
	public function register($username, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$this->_mysql->insert($this->tables['users'], $user_data);
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->_mysql->select($this->identity_column . ', username, email, id, password, active, last_login')
		                  ->where($this->identity_column, $this->_mysql->escape_str($identity))
		                  ->limit(1)
		                  ->get($this->tables['users']);
	...
	function get_attempts_num($identity)
	{
		if ($this->config->item('track_login_attempts', 'ion_auth')) {
			...
			$this->_mysql->select('1', FALSE);
			$this->_mysql->where('ip_address', $ip_address);
			if (strlen($identity) > 0) $this->_mysql->or_where('login', $identity);

			$qres = $this->_mysql->get($this->tables['login_attempts']);
	...
	public function get_last_attempt_time($identity) {
		if ($this->config->item('track_login_attempts', 'ion_auth')) {
			...
			$this->_mysql->select_max('time');
			$this->_mysql->where('ip_address', $ip_address);
			if (strlen($identity) > 0) $this->_mysql->or_where('login', $identity);
			$qres = $this->_mysql->get($this->tables['login_attempts'], 1);
	...
	public function increase_login_attempts($identity) {
		if ($this->config->item('track_login_attempts', 'ion_auth')) {
			$ip_address = $this->_prepare_ip($this->input->ip_address());
			return $this->_mysql->insert($this->tables['login_attempts'], array('ip_address' => $ip_address, 'login' => $identity, 'time' => time()));
		}
		return FALSE;
	}
	...
	public function clear_login_attempts($identity, $expire_period = 86400) {
		if ($this->config->item('track_login_attempts', 'ion_auth')) {
			...
			$this->_mysql->where(array('ip_address' => $ip_address, 'login' => $identity));
			// Purge obsolete login attempts
			$this->_mysql->or_where('time <', time() - $expire_period, FALSE);

			return $this->_mysql->delete($this->tables['login_attempts']);
		}
		return FALSE;
	}
	...
	public function users($groups = NULL)
	{
		...
		if (isset($groups))
		{
			...
			if (isset($groups) && !empty($groups))
			{
				...
				$this->_mysql->where_in($this->tables['users_groups'].'.'.$this->join['groups'], $groups);
			}
		}
		...
		{
			$this->_mysql->limit($this->_ion_limit, $this->_ion_offset);
		...
		{
			$this->_mysql->limit($this->_ion_limit);
		...
		{
			$this->_mysql->order_by($this->_ion_order_by, $this->_ion_order);

			$this->_ion_order    = NULL;
			$this->_ion_order_by = NULL;
		}

		$this->response = $this->_mysql->get($this->tables['users']);
	...
	public function get_users_groups($id=FALSE)
	{
		...
		return $this->_mysql->select($this->tables['users_groups'].'.'.$this->join['groups'].' as id, '.$this->tables['groups'].'.name, '.$this->tables['groups'].'.description')
	...
	public function add_to_group($group_id, $user_id=false)
	{
		...
		if ($this->_mysql->where(array( $this->join['groups'] => (int)$group_id, $this->join['users'] => (int)$user_id))->get($this->tables['users_groups'])->num_rows()) return false;

		if ($return = $this->_mysql->insert($this->tables['users_groups'], array( $this->join['groups'] => (int)$group_id, $this->join['users'] => (int)$user_id)))
		{
	...
	public function remove_from_group($group_ids=false, $user_id=false)
	{
		...
		if( ! empty($group_ids))
		{
			...
			{
				$this->_mysql->delete($this->tables['users_groups'], array($this->join['groups'] => (int)$group_id, $this->join['users'] => (int)$user_id));
		...
		{
			if ($return = $this->_mysql->delete($this->tables['users_groups'], array($this->join['users'] => (int)$user_id))) {
	...
	public function groups()
	{
		...
		$this->response = $this->_mysql->get($this->tables['groups']);
	...
	public function group($id = NULL)
	{
		...
		if (isset($id))
		{
			$this->_mysql->where($this->tables['groups'].'.id', $id);
		}
	...
	public function update($id, array $data)
	{
		...
		$this->_mysql->trans_begin();

		if (array_key_exists($this->identity_column, $data) && $this->identity_check($data[$this->identity_column]) && $user->{$this->identity_column} !== $data[$this->identity_column])
		{
			$this->_mysql->trans_rollback();
		...
		$this->trigger_events('extra_where');
		$this->_mysql->update($this->tables['users'], $data, array('id' => $user->id));

		if ($this->_mysql->trans_status() === FALSE)
		{
			$this->_mysql->trans_rollback();
	...
	**/
	public function delete_user($id)
	{
		...
		$this->_mysql->delete($this->tables['users'], array('id' => $id));
	...
	public function update_last_login($id)
	{
		...
		$this->_mysql->update($this->tables['users'], array('last_login' => time()), array('id' => $id));
	...
	public function remember_user($id)
	{
		...
		$this->_mysql->update($this->tables['users'], array('remember_code' => $salt), array('id' => $id));

		if ($this->_mysql->affected_rows() > -1)
		{
			// if the user_expire is set to zero we'll set the expiration two years from now.
			if($this->config->item('user_expire', 'ion_auth') === 0)
			{
	...
	public function login_remembered_user()
	{
		...
		//get the user
		$this->trigger_events('extra_where');
		$query = $this->_mysql->select($this->identity_column.', id, username, email, last_login')
	...
	*/
	public function create_group($group_name = FALSE, $group_description = '', $additional_data = array())
	{
		...
		$existing_group = $this->_mysql->get_where($this->tables['groups'], array('name' => $group_name))->num_rows();
		if($existing_group !== 0)
		{
		...
		$this->_mysql->insert($this->tables['groups'], $data);
		$group_id = $this->_mysql->insert_id();
	...
	public function update_group($group_id = FALSE, $group_name = FALSE, $additional_data = array())
	{
		...
		if (!empty($group_name))
		{
			...
			// bail if the group name already exists
			$existing_group = $this->_mysql->get_where($this->tables['groups'], array('name' => $group_name))->row();
		...
		$this->_mysql->update($this->tables['groups'], $data, array('id' => $group_id));
	...
	**/
	public function delete_group($group_id = FALSE)
	{
		...
		$this->_mysql->delete($this->tables['users_groups'], array($this->join['groups'] => $group_id));
		// remove the group itself
		$this->_mysql->delete($this->tables['groups'], array('id' => $group_id));
	...
	{
		$filtered_data = array();
		$columns = $this->_mysql->list_fields($table);
	...
	protected function _prepare_ip($ip_address) {
		if ($this->_mysql->platform() === 'postgre' || $this->_mysql->platform() === 'sqlsrv' || $this->_mysql->platform() === 'mssql')
		{
			return $ip_address;
		}
		else
