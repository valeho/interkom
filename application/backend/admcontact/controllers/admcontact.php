 ...
 * @author Виктор
 */
class Admcontact extends MX_Controller
{
    ...
    public function index()
    {
        ...
        $result = sys_contact::find('all', array('conditions' => array('is_delete = ?', 0)));
        ...
        if ($this->input->post("save", TRUE) == 'Y')
        {
            ...
            try
            {
                ...
                if (!$sys_data->save())
                {
                    ...
                    sys_contact::connection()->rollback();
                }
                else
                {
                    sys_contact::connection()->commit();
                    redirect(site_url() . 'admcontact/index', 'refresh');
                }
            }
            catch (Exception $ex)
            {
    ...
    public function edit($id = 0)
    {
        ...
        if ($this->input->post("save", TRUE) == 'Y')
        {
            $sys_data = sys_contact::find('first', array('conditions' => array('id= ?', (int) $id)));
            if (is_null($sys_data))
            {
                redirect(site_url() . 'admcontact/index', 'refresh');
            }
        ...
        $result = sys_contact::find('first', array('conditions' => array('id= ?', (int) $id)));
        if (is_null($result))
        {
            redirect(site_url() . 'admcontact/index', 'refresh');
        }
